...
function checkInvariants(
  snap: ReturnType<Match['getSnapshot']>,
  prev: { explored: number; events: number; tick: number },
  violations: string[],
  where: string,
): { explored: number; events: number; tick: number } {
  ...
  return { explored: prev.explored, events: prev.events, tick };
}
...
function playGame(seed: number): { stats: GameStats; violations: string[]; csv: string[] } {
  ...
  let guard = { explored: 0, events: 0, tick: 0 };
  let rid = 0;
...
function main(): void {
  ...
  const report = {
    ...
    totals: { applied, rejected, violations: violations.length },
    balance: {
      avgTicks: avgTick,
      avgFinalP1: {
