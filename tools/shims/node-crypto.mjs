// Browser shim for the engine's two node:crypto uses (randomUUID, SHA-256).
// randomUUID is native in browsers; SHA-256 below is a compact sync
// implementation, verified byte-for-byte against node:crypto (see
// tools/shims/verify-sha256.mjs). No dependencies.
const K = [
...
export function createHash(algorithm) {
  if (algorithm !== 'sha256') throw new Error('shim createHash supports sha256 only');
  const chunks = [];
  return {
